      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center py-4">
          ...
          <div className="hidden lg:flex items-center space-x-8">
            <nav className="flex space-x-6">
              ...
              <Link href="/about" className={`font-medium ${location === '/about' ? 'text-primary' : 'hover:text-primary'} transition`}>
                About
              </Link>
              <div className="relative group">
                <button 
                  className={`font-medium ${location.startsWith('/services') ? 'text-primary' : 'hover:text-primary'} transition flex items-center`}
                >
                  Services <i className="fas fa-chevron-down ml-1 text-xs"></i>
                </button>
                <div className="absolute left-0 mt-2 w-56 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition duration-200 bg-white shadow-lg rounded-md py-2 z-50">
              ...
              <Link href="/case-studies" className={`font-medium ${location === '/case-studies' ? 'text-primary' : 'hover:text-primary'} transition`}>
                Case Studies
              </Link>
              <Link href="/blog" className={`font-medium ${location === '/blog' ? 'text-primary' : 'hover:text-primary'} transition`}>
                Insights
              </Link>
              <Link href="/contact" className={`font-medium ${location === '/contact' ? 'text-primary' : 'hover:text-primary'} transition`}>
                Contact
              </Link>
            </nav>
        ...
        <div className={`lg:hidden ${isOpen ? 'block' : 'hidden'} transition-all duration-300 ease-in-out`}>
          <div className="py-4 space-y-4 border-t border-gray-200">
            <Link href="/" className="block font-medium py-2">Home</Link>
            <Link href="/about" className="block font-medium py-2">About</Link>
            ...
            <div className="relative">
              ...
              >
                Services 
                <i className={`fas fa-chevron-${servicesOpen ? 'up' : 'down'} text-xs`}></i>
              </button>
              <div className={`${servicesOpen ? 'block' : 'hidden'} pl-4 pt-2 space-y-2`}>
                {services.map((service) => (
                  <Link 
                    key={service.slug}
                    href={`/services/${service.slug}`} 
